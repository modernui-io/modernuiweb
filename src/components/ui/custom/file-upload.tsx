...
// Ref: https://shadcn-extension.vercel.app/docs/file-upload
import {
  createContext,
  forwardRef,
  useCallback,
...
>(
  (
        ...
        if (rejectedFiles.length > 0) {
          ...
          for (let i = 0; i < rejectedFiles.length; i++) {
            ...
            if (rejectedFiles[i]?.errors[0]?.message) {
              toast.error(
                rejectedFiles[i]?.errors[0]?.message ?? "Unsuccessful upload",
              );
              break;
            }
          }
    ...
    const handleKeyDown = useCallback(
      ...
      [value, activeIndex, dropzoneState.inputRef, removeFileFromSet],
    );
